...
import { renderCOVIDCards } from '../../actions/cardsActions';
import styles from './COVIDCards.module.css';

const COVIDCards = () => {

    const cardsAlpha = useSelector((state) => state.cardsAlpha);
    const dispatch = useDispatch();

    useEffect(() => {
        dispatch(renderCOVIDCards())
    }, [dispatch]);

    // if (!cardsAlpha.confirmed) {
    //     return <Button variant="primary" disabled>
    //         <Spinner as="span" animation="border" size="sm" role="status" aria-hidden="true" justify-content="center" />
    //        Loading...
    //        </Button>
    // }
    ...
    return (
        <>
            ...
            <Jumbotron style={{ background: "#add8e6" }}>
                <div class="text-black font-weight-bold">
                    <h1> <u> <center>Global COVID-19 Statistics </center> </u> </h1>
                    <div class="row">
                        <Card style={{ width: '18rem' }}>
                            ...
                            <Card.Body>
                                <Card.Title><CountUp start={0} end={cardsAlpha.deaths} duration={3.0} separator="," /></Card.Title>
                                <Card.Text>{new Date(cardsAlpha.date).toDateString()}</Card.Text>
                        ...
                        <Card style={{ width: '18rem' }}>
                            ...
                            <Card.Body>
                                <Card.Title><CountUp start={0} end={cardsAlpha.confirmed} duration={3.0} separator="," /></Card.Title>
                                <Card.Text>{new Date(cardsAlpha.date).toDateString()}</Card.Text>
                        ...
                        <Card style={{ width: '18rem' }}>
                            ...
                            <Card.Body>
                                <Card.Title><CountUp start={0} end={cardsAlpha.recovered} duration={3.0} separator="," /> </Card.Title>
                                <Card.Text>{new Date(cardsAlpha.date).toDateString()}</Card.Text>
                                <Card.Text> *Total current, active cases since date specified above* </Card.Text>
                            </Card.Body>
                        </Card>
